...
return {
    ...
    tests = {
        ...
        {
            name = "Lunge uses configured speed (20 studs) and exposes test hooks",
            fn = function()
                ActionController.Character = { FindFirstChild = function(_, _) return nil end }
                ActionController.Humanoid = { Health = 100 }

                -- ensure config default is 20
                assert(type(MovementConfig) == "table" and MovementConfig.Movement and MovementConfig.Movement.LungeSpeed == 20,
                    "MovementConfig LungeSpeed should be 20")

                -- fake root so _PlayActionLocal runs without error
                local fakeRoot = Instance.new("Part")
                fakeRoot.Anchored = false
                fakeRoot.CanCollide = false
                ...
                -- _Test_LastLungeSpeed should equal config value
                assert(ActionController._Test_LastLungeSpeed == 20,
                    "Test hook should expose the lunge speed used")
                assert(ActionController._Test_LastLungeKeepTime and ActionController._Test_LastLungeKeepTime > 0,
                    "Keep time should be recorded")
