# GitHub Issues for Optimization Epic

This document contains ready-to-use GitHub issue templates for all 26 sub-issues of the Performance Optimization Epic.

Use the `gh` CLI tool to create these issues:

```bash
gh issue create --repo JoshuaKushnir/Nightfall --title "TITLE" --label "LABELS" --body "BODY"
```
...
gh issue create \
  ...
  --title "Service require refactoring" \
  --label "phase-1,backend,tech-debt,high" \
  --body "$(cat <<'EOF'
## Overview
...
## Epic
Part of Performance Optimization Epic (see docs/OPTIMIZATION-EPIC.md)
EOF
)"
```
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Attribute caching in tight loops" \
  --label "phase-1,backend,performance,high" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Centralized tick loops" \
  --label "phase-1,infrastructure,refactor,medium" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Movement state machine caching" \
  --label "phase-1,backend,performance,medium" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Debug systems gating" \
  --label "phase-1,infrastructure,tech-debt,low" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Module initialization audit" \
  --label "phase-1,infrastructure,audit,low" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Hitbox lifetime & size tuning" \
  --label "phase-2,combat,performance,high" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Batch hitbox checks for PvE" \
  --label "phase-2,combat,performance,medium" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Movement state machine optimization" \
  --label "phase-2,backend,performance,medium" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "CombatService per-hit optimization" \
  --label "phase-2,combat,performance,high" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Minimal cheap VFX implementations" \
  --label "phase-3,frontend,performance,medium" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Particle emitter reuse pool" \
  --label "phase-3,frontend,performance,medium" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Distance-based ambient culling" \
  --label "phase-3,frontend,performance,low" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "LOD & polygon optimization" \
  --label "phase-3,frontend,performance,medium" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Rendering profiling report" \
  --label "phase-3,frontend,documentation,low" \
  --body "$(cat <<'EOF'
## Overview
...
## Dependencies
Blocked by: #200, #201, #202, #203, #204
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Event-level snapshot architecture" \
  --label "phase-4,networking,infrastructure,high" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Data quantization for compression" \
  --label "phase-4,networking,performance,medium" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Rate-limited logging & debug events" \
  --label "phase-4,networking,performance,low" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Ability activation consolidation" \
  --label "phase-4,networking,performance,medium" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Client-side prediction for simple actions" \
  --label "phase-4,networking,frontend,low" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "UI binding optimization" \
  --label "phase-5,frontend,ui,medium" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "Animation preloading & asset validation" \
  --label "phase-5,frontend,animation,low" \
  --body "$(cat <<'EOF'
## Overview
...
gh issue create \
  --repo JoshuaKushnir/Nightfall \
  --title "HUD update queue & batching" \
  --label "phase-5,frontend,ui,low" \
  --body "$(cat <<'EOF'
## Overview
...
To create all issues at once, save the above commands to separate shell scripts or use a loop:
...
# After Phase 1 completes, create Phase 2
# ... etc
```

Or batch create (adjust for your shell):

```bash
# Create all Phase 1 issues
for issue in 189 190 191 192 193 194; do
  echo "Creating issue #$issue..."
  # Run corresponding gh command
done
```
...
The issue numbers (#189-#214) are referenced in the epic but will be determined by GitHub when created. After creation, update the OPTIMIZATION-EPIC.md file with actual issue numbers.

To link issues as blocking relationships, use GitHub web UI:
1. Open each dependent issue
2. Click "Linked issues" in sidebar
3. Add "blocked by" relationship to dependencies
