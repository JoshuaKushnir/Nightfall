...
function DummyService:Start()
    ...
    Players.PlayerAdded:Connect(function(player)
        ...
        if spawnEvt then
            for _, dd in pairs(ActiveDummies) do
...
function DummyService._CreateDummyModel(dummyData: DummyData): Model?
    ...
    local function makeBar(name: string, yOffset: number, color3: Color3)
...
function DummyService._UpdateVisuals(dummyId: string)
    ...
    -- Update BillboardGui via stored HUD references
    if dummyData._Hud then
        local stateLabel = dummyData._Hud.StateLabel
        if stateLabel then
            stateLabel.Text = dummyData.State
        end

        local healthBar = dummyData._Hud.HealthBar
        if healthBar then
            healthBar.Size = UDim2.new(dummyData.Health / dummyData.MaxHealth, 0, 0.2, 0)
        end

        local postureBar = dummyData._Hud.PostureBar
        if postureBar and dummyData.Posture and dummyData.MaxPosture then
            postureBar.Size = UDim2.new(dummyData.Posture / dummyData.MaxPosture, 0, 0.2, 0)
        end
    end
...
function DummyService.SetDummyState(dummyId: string, state: DummyState)
    ...
    if state == "Staggered" then
        task.delay(STAGGER_DURATION, function()
            ...
            if d and d.State == "Staggered" and d.IsActive then
...
function DummyService._ApplyKnockback(dummyId: string, attackerPosition: Vector3)
    ...
    for _, part in ipairs(model:GetDescendants()) do
        if part:IsA("BasePart") then
            local tween = TweenService:Create(part, tweenInfo, { CFrame = part.CFrame + kbOffset })
            tween:Play()
        end
    end
...
function DummyService._StartBlockingCycle(dummyId: string)
    task.spawn(function()
        while ActiveDummies[dummyId] do
            ...
            local d = ActiveDummies[dummyId]
            if not d or not d.IsActive then break end
            ...
            if d.State == "Normal" then
                ...
                local d2 = ActiveDummies[dummyId]
                if d2 and d2.State == "Blocking" and d2.IsActive then
                    DummyService.SetDummyState(dummyId, "Normal")
                end
            end
        end
...
Currently invoked manually from server code.
]]
...
function DummyService.ApplyDamage(dummyId: string, damage: number, attackerPosition: Vector3?): boolean
    ...
    print(
        `[DummyService] Dummy hit: {dummyId} ({effectiveDamage} dmg, {dummyData.Health}/{dummyData.MaxHealth} HP)`
    )
...
function DummyService.GetAllDummyData(): {[string]: DummyData}
    ...
    for k, v in pairs(ActiveDummies) do
        copy[k] = v
    end
    return copy
end
...
function DummyService._CleanupAllDummies()
    for dummyId in pairs(ActiveDummies) do
        DummyService.DespawnDummy(dummyId)
    end
end
