...
local function _TickWitnessing(dt: number)
	...
	for _, player in Players:GetPlayers() do
		...
		if not currentTrack or currentTrack.ConfigId ~= bestConfig then
			...
			if progEvent then
				progEvent:FireClient(player, { TargetInstanceId = bestInstanceId, Progress = currentTrack.TimeAccrued / WITNESS_TIME_REQUIRED })
			end
			...
			if currentTrack.TimeAccrued >= WITNESS_TIME_REQUIRED then
				...
				-- Notify client
				local unlockEvent = NetworkProvider:GetRemoteEvent("CodexUnlocked")
				if unlockEvent then
					unlockEvent:FireClient(player, { EntryId = bestConfig, Title = "Witnessed: " .. bestConfig })
				end
			end
		end
	end
