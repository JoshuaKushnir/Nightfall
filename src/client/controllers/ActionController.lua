...
--[[
	Determine the roll folder and asset names based on movement direction
	@param moveDir World-space movement direction
...
function ActionController.PlayAction(config: ActionConfig)
	print(`[ActionController] PlayAction called: {config.Name}`)
	
	-- For dodge actions, determine roll direction dynamically
	if config.Type == "Dodge" and MovementController then
		local moveDir = MovementController:GetLastMoveDirection()
		...
		-- For light attacks, handle combo system
...
function ActionController._PlayActionLocal(config: ActionConfig)
	...
	print(`[ActionController] _PlayActionLocal: {config.Name}`)
	...
	elseif not Humanoid then
		...
		if track then
			...
			print(`[ActionController] ✗ No animation track loaded for {config.Name}. AnimationName: {config.AnimationName or "nil"}, AnimationId: {config.AnimationId or "nil"}`)
		end
	end
