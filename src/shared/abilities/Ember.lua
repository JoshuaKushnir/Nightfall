...
local function _applyHeatStack(
    ...
    if tPlayer then
        _getPostureService().GainPosture(tPlayer, IGNITE_POSTURE_PER_HEAT * stacks)
    end
    ...
    elseif dummyId then
        _getDummyService().ApplyDamage(dummyId, IGNITE_HP_DMG * stacks, casterPos)
    end
    ...
    if newStacks >= HEAT_STACK_MAX then
        ...
        task.spawn(function()
            while char and char.Parent do
                ...
                if tPlayer then
                    if CS then CS.ApplyBreakDamage(tPlayer, BURNING_HP_PER_SEC) end
                elseif dummyId then
                    _getDummyService().ApplyDamage(dummyId, BURNING_HP_PER_SEC, nil)
                end
            end
