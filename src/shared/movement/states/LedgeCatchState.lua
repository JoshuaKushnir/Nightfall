...
--[[
	...
	Public surface:
	  ...
	  Update(dt, ctx)   — empty; ledge logic runs fully inside TryStart's task.spawn / conn.
...
--[[
	Called every frame from MovementController when airborne and not movement-restricted.
	Probes for a ledge edge above the character and initiates catch if conditions are met.
]]
...
function LedgeCatchState.TryStart(ctx: any)
	...
	local wallHit = workspace:Raycast(wallRayOrigin, probe.lookDir * 5, params)
	
	local catchPos
	if wallHit then
		catchPos = wallHit.Position + wallHit.Normal * 1.1
	...
	-- Hang indefinitely until PullUp() is called (Space key via MovementController._OnJumpRequest).
	-- Safety fallback only: auto-release after 30 seconds to prevent permanent hang.
...
function LedgeCatchState.PullUp(ctx: any)
	...
	-- Prevent immediate pull-up if the player is just holding the jump button
...
function LedgeCatchState.Update(_dt: number, _ctx: any)
	-- Hang and pull-up logic runs in PullUp / timeout.
end
